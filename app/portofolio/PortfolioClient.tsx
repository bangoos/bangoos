...
import type { Database, PortfolioItem } from "@/lib/types";
...
export default function PortfolioClient({ db }: PortfolioClientProps) {
  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-6 max-w-6xl w-full py-12">
          ...
          {db.portfolio.map((item) => (
            <div key={item.id} className="group bg-gradient-to-b from-[#1a2332] to-[#0F1628] border border-gray-700/50 rounded-2xl overflow-hidden hover:-translate-y-2 transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/20">
              ...
              <div className="p-6">
                ...
                <button onClick={() => setSelectedItem(item)} className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-400 hover:text-cyan-300 transition-colors">
        ...
        <PortfolioModal open={!!selectedItem} onClose={() => setSelectedItem(null)} item={selectedItem} />
      </div>
    </PageLayout>
  );
}
